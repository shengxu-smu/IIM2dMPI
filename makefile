OBJ = allocateEuler.o Cartesiangrid.o cfl.o coords2rank.o\
      Euler.o exchange4ghost.o exchangeobject.o\
      indexrange.o initial.o interpolateuv.o Lagrange.o loadobject.o locateobject.o\
      main.o moveobject.o outputEuler.o panelinPE.o para.o \
      readpara.o rhs4p.o rk.o setuphypre.o solvep.o startMPI.o stretchxy.o old_save.o\
      ubc.o vbc.o pbc.o rhs4uv.o updateuv.o data_plot.o run_output.o\
      velocity_initial.o Raycrossing.o surface_property.o jc_firstsecond.o interpolate.o FiniteDiff.o\
      principle_jc_p.o allocate_jcs.o mac_distribute.o correction_interpolate.o jump_contribution.o\
			...
			correction_pressure.o velocity_reset.o free_jcs.o jc_pressure.o\
			rhs4p_clean.o Euler_reset.o Divergence_reset.o
...
# Miguel
HYPRE = /home/yang/Downloads/hypre-2.11.1/src/hypre
# HYPRE = /Users/YangLiu/Desktop/hypre-2.11.0/src/hypre
#iMac
#HYPRE = /Users/yang/Desktop/hypre/hypre-2.10.1/src/hypre
...
exchangeobject.o: $(MOD3) panelinPE.f90 exchangeobject.f90
...
pbc.o: $(MOD1) pbc.f90
	$(FC) -c $(FLAG) pbc.f90
FiniteDiff.o: $(MOD1) FiniteDiff.f90
	$(FC) -c $(FLAG) FiniteDiff.f90
allocate_jcs.o: $(MOD3) allocate_jcs.f90
	$(FC) -c $(FLAG) allocate_jcs.f90
jc_firstsecond.o: $(MOD3) jc_firstsecond.f90 interpolate.f90 FiniteDiff.f90
	$(FC) -c $(FLAG) jc_firstsecond.f90
principle_jc_p.o: $(MOD3) principle_jc_p.f90 interpolate.f90 FiniteDiff.f90
	$(FC) -c $(FLAG) principle_jc_p.f90
surface_property.o: $(MOD2) surface_property.f90
...
dudv_surface.o: $(MOD3) dudv_surface.f90 interpolate.f90 FiniteDiff.f90
...
jump_contribution.o: $(MOD3) jump_contribution.f90 correction_interpolate.f90 interpolateuv.f90\
	 													 correction_strain.f90 jc_pressure.f90\
...
velocity_initial.o: $(MOD3) velocity_initial.f90 solvep.f90 exchange4ghost.f90 principle_jc_p.f90 \
														...
														pressure_distribute.f90 velocity_reset.f90 free_jcs.f90
...
data_plot.o:$(MOD3) data_plot.f90
	$(FC) -c $(FLAG) data_plot.f90
run_output.o:$(MOD3) run_output.f90
	$(FC) -c $(FLAG) run_output.f90
main.o: $(MOD3) readpara.f90 startMPI.f90 Cartesiangrid.f90 \
                locateobject.f90 loadobject.f90 allocateEuler.f90 \
                setuphypre.f90 initial.f90 run_output.f90 \
                cfl.f90 rk.f90 outputEuler.f90 data_plot.f90 mac_distribute.f90\
                main.f90
	$(FC) -c $(FLAG) main.f90
moveobject.o: $(MOD2) moveobject.f90
	$(FC) -c $(FLAG) moveobject.f90
outputEuler.o: $(MOD1) outputEuler.f90
...
rhs4p.o: $(MOD1) rhs4p.f90 pbc.f90 Divergence_reset.f90
	$(FC) -c $(FLAG) rhs4p.f90
...
rk.o: $(MOD3) rk.f90 old_save.f90 uv_strain.f90 rhs4p.f90 surface_property.f90 Raycrossing.f90\
							...
							jump_contribution.f90 solvep.f90 rhs4uv.f90 updateuv.f90 free_jcs.f90\
							exchange4ghost.f90 interpolateuv.f90 Euler_reset.f90 pressure_distribute.f90
	$(FC) -c $(FLAG) rk.f90
setuphypre.o: $(MOD1) setuphypre.f90
	$(FC) -c $(FLAG) setuphypre.f90
rhs4p_clean.o: $(MOD3) rhs4p_clean.f90
	$(FC) -c $(FLAG) rhs4p_clean.f90
solvep.o: $(MOD1) solvep.f90 rhs4p_clean.f90
	$(FC) -c $(FLAG) solvep.f90
