SUBROUTINE jc_firstsecond
  ...
  DO myobj=1,nobj4proc
     ...
     DO myvertex=nvertex4proc(myobj-1)+1,nvertex4proc(myobj)
        ...
        ENDIF
        corner0=(/xf(iubeg-2),yf(jvbeg-2)/)
        corner1=(/xf(iuend+2),yf(jvend+2)/)
        ...
        IF( (A(1)>corner0(1)).AND.(A(2)>corner0(2)).AND. &
           ...
           jcp1 = SIN(A(1))*SIN(A(2))-EXP(-A(1))*EXP(-A(2))
           jcp2 = SIN(B(1))*SIN(B(2))-EXP(-B(1))*EXP(-B(2))
           dist = SQRT((B(1)-A(1))*(B(1)-A(1))+(B(2)-A(2))*(B(2)-A(2)))
           dpdt = (jcp2-jcp1)/dist

           dpdt = COS(xx(0))*SIN(yy(0))*tx+SIN(xx(0))*COS(yy(0))*ty + &
                  EXP(-xx(0))*EXP(-yy(0))*(tx+ty)

           dpdnp = COS(xx(0))*SIN(yy(0))*xn+SIN(xx(0))*COS(yy(0))*yn
           dpdnn = -EXP(-xx(0))*EXP(-yy(0))*(xn+yn)
           dpdnjc = dpdnp-dpdnn
     ...
     DO myvertex=nvertex4proc(myobj-1)+1,nvertex4proc(myobj)-1
        ...
        IF( (A(1)>corner0(1)).AND.(A(2)>corner0(2)).AND. &
           ...
           !  if(myid.eq.0) print*,myid,myvertex,pjc(1,myvertex),pjc(1,myvertex+1)

           ! print*,myid,myvertex,dpdxdt,pjc(1,myvertex+1),pjc(1,myvertex)
           ! ddpdx ddpdy jump conditions
           r3 = -2.0d0*SIN(A(1))*SIN(A(2))-2.0d0*EXP(-A(1))*EXP(-A(2))
           ...
           ! print*,myvertex,ddpyjc0,dpdxdt,dpdydt
           !  print*,myid,myvertex,pjc(2,myvertex+1),pjc(2,myvertex),dpdydt,dist
           ...
           dpdxtrue = -SIN(A(1))*SIN(A(2))-EXP(-A(1))*EXP(-A(2))
           dpdytrue = -SIN(A(1))*SIN(A(2))-EXP(-A(1))*EXP(-A(2))
  ...
  DO myobj=1,nobj4proc
     ...
     DO myvertex=nvertex4proc(myobj),nvertex4proc(myobj-1)+2,-1
        ...
        IF( (A(1)>corner0(1)).AND.(A(2)>corner0(2)).AND. &
           ...
           !  print*,myvertex,ddpxjc0,ddpyjc0
  ...
  DO myobj=1,nobj4proc
     ...
     DO myvertex=nvertex4proc(myobj-1)+1,nvertex4proc(myobj)
        ...
        IF( (A(1)>corner0(1)).AND.(A(2)>corner0(2)).AND. &
           ...
           dpdxtrue = COS(xx(0))*SIN(yy(0))+EXP(-xx(0))*EXP(-yy(0))
           dpdytrue = SIN(xx(0))*COS(yy(0))+EXP(-xx(0))*EXP(-yy(0))
     ...
     DO myvertex=nvertex4proc(myobj-1)+1,nvertex4proc(myobj)
        ...
        IF( (A(1)>corner0(1)).AND.(A(2)>corner0(2)).AND. &
           ...
           !  PRINT*,myid,myvertex,pjc(7,myvertex),dpdxtrue,pjc(8,myvertex),dpdytrue
           !  PRINT*,myid,myvertex,vertex(1,myvertex),vertex(2,myvertex),vertex(1,myvertex+1),vertex(2,myvertex+1)
           !  PRINT*,myid,myvertex,corner0(1),corner1(1),corner0(2),corner1(2)
